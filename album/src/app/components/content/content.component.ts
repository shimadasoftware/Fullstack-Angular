...
export class ContentComponent implements OnInit {
  ...
  public modalStart: number;
  public modalEnd: number;
  ...
  public updatePainting(): void {
    if (this.paintingSelected.endingYear < this.paintingSelected.startYear || this.paintingSelected.description == ''
      || this.paintingSelected.paintingName == '') {
      const parameters = {
        closeButton: true,
        enableHtml:true,
        progressBar:true,
      ...
      this.paintingSelected = new Painting(0, 0, 0, '', '', '', '');
      this.modalRef.hide();
      this.toastr.success('In good time, the fields have been successfully updated. ', 'SUCCESS');
    }
  }
  ...
  public savePainting(): boolean {
    if (this.paintingSelected.paintingName == '' || this.paintingSelected.description == ''
      ...
      this.toastr.success('In good time, the fields have been created successfully. ', 'SUCCESS');
    }
      return false;
  }
  ...
  public deletePainting(objPainting: Painting): void {
      ...
      this.paintingSelected = new Painting(0, 0, 0, '', '', '', '');
      this.toastr.success('In good time, the fields have been deleted successfully. ', 'SUCCESS');
  }
  ...
  public openModalCreate(template: TemplateRef<any>):void {
    this.modalRef = this.modalService.show(template, {class:'modal-md'});
    this.modalTitle = 'Create';
    this.modalText = 'Create a new series of painting:';
    this.modalContent = this.paintingSelected.paintingName;
  }

  public openModalDescription(template: TemplateRef<any>, tempPainting: Painting):void {
    this.paintingSelected = tempPainting;
    this.modalRef = this.modalService.show(template, {class:'modal-md'});
    this.modalTitle = 'More information';
    this.modalContent = this.paintingSelected.paintingName;
    this.modalStart = this.paintingSelected.startYear;
    this.modalEnd = this.paintingSelected.endingYear;
    this.modalText = this.paintingSelected.description;
  }
}
